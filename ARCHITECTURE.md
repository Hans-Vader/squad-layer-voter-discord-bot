...
**SquadCalc integration** (utils.py:55-275): `build_squadcalc_url` produces a parameterized SquadCalc deep link **only for `source == "main"`** (the SquadCalc-compatible source). For SPM/SU/supermod layers, `build_map_icon_markdown` renders the 🗺️ icon as a masked link that exists only to carry a hover tooltip (map + version + full faction names); its target is `_TOOLTIP_NOOP_URL`, which is SquadCalc's bare homepage — a dead end by design, and the reason the footer counts it as no destination. Not merely to avoid a useless link: an unmatched `map` parameter makes SquadCalc pick a **random map** and strip the rest (`squadCalc.js:489-493`), so a guessed name is worse than no link. This is why the restriction survives SquadCalc gaining mod support — `supermod_layers.json` cannot yield SquadCalc's mod map and layer names (its `mapName` omits GoingDark on some layers, and PreCap variants collapse onto the same derived layer name). When `SQUADCALC_BASE_URL` is empty, main-source layers fall back to a plain emoji. A suggestion carrying a `workshop_url` (custom maps only, see below) outranks both: the icon points at the mod's Steam Workshop page, since SquadCalc has no data for admin-defined maps. That precedence governs four surfaces, kept in one order so they can never disagree: the icon, the derived footer below, `bot._layer_link_line` (the "Open in …" line on the suggest confirmation and the remove-suggestion prompt), and `build_winner_copy_text`, whose escaped `\[Steam Workshop\](url)` / `\[SquadCalc\](url)` survives a select-copy into a Discord scheduled event.
