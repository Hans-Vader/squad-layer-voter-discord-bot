...
> **A unit's vehicle list is layer-independent.** `Units[].vehicles` is the unit's full pool; which of it actually spawns depends on the *layer's* vehicle spawners, and **no per-layer spawner data exists in these files** (nor anywhere in the upstream exporter, which only reads `FactionSetup`). So the raw list shows RHIBs on waterless maps and helis on layers with no heli pad. `_MAP_ABSENT_SPAWNERS` in `bot/bot.py` trims this at *map* granularity — maps that have no boat/helicopter spawner on any of their layers (derived from the Squad-Wiki map-data pipeline). Per-layer gaps still show through.
